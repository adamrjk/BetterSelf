...
struct ClickIndicator: View {
    ...
    private func updatePosition(in geometry: GeometryProxy) {
        if let targetId = targetViewId {
            // Smart positioning based on target view
            let newPosition = viewFinder.calculateOptimalIndicatorPosition(
                for: targetId,
                in: geometry.size
            )
            indicatorPosition = newPosition
            indicatorDirection = viewFinder.getIndicatorDirection(for: targetId, indicatorPosition: newPosition)
